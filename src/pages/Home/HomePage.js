...
//   const [feed, setFeed] = useState({ results: [] }); // Combined posts and milestones
...
//       try {
//         const [postsData, milestonesData] = await Promise.all([
//           axiosReq.get(`/posts/?search=${query}`),
//           axiosReq.get(`/milestones/?search=${query}`),
//         ]);

//         // Combine posts and milestones, adding "type" field for differentiation
//         const combinedFeed = [
//           ...postsData.data.results.map((post) => ({ ...post, type: "post" })),
//           ...milestonesData.data.results.map((milestone) => ({
//             ...milestone,
//             type: "milestone",
//           })),
